...
public class manager extends JDBCinfo{
	public void createANewBookingUser(String name, String idnum, String room, int roomid, String from, String to, String status) {
		try{
			...
			generateAndSendEmail("358456898@qq.com","booking",name,idnum,room,from,to);
		}
		catch (Exception exc) {
			exc.printStackTrace();
		}
		...
		try{
			...
			while (myRs4.next()) {
				if(name.equals(myRs4.getString("name"))){
					...
					generateAndSendEmail("358456898@qq.com","Cancel",myRs4.getString("name"),myRs4.getString("idnum"),myRs4.getString("room"),myRs4.getString("fromday"),myRs4.getString("today"));
					return true;
					}
				}	
			}
